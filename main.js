...
const firebaseConfig = {
  ...
  storageBucket: "mift-user-profile.appspot.com", // مهم جداً
...
function setupNav(user) {
  ...
  navItems.forEach(btn => {
    btn.onclick = () => {
      ...
      if (target === "employeesScreen") loadEmployees();
      if (target === "assetsScreen") loadEmployeesSelect();
      if (target === "storesScreen") loadStores();
      if (target === "usersAdminScreen" && user.isSuperAdmin) loadUsersList();
    };
  });

  if (user.isSuperAdmin && adminBtn) {
    adminBtn.click();
  } else if (employeesBtn) {
    employeesBtn.click();
  }
}
...
// تصدير لدوال الحذف/التعديل للـ HTML
