...
/********************************************
 * ROLE-BASED NAVIGATION  (FINAL FIXED VERSION)
 ********************************************/
function setupNav(user) {
  ...
  if (user.isSuperAdmin) {
    ...
    adminScreenBtn.classList.add("active");
    document.getElementById("usersAdminScreen").classList.remove("hidden");
  ...
  if (user.role === "admin") {
    navItems.forEach(btn => btn.style.display = "block");
    adminScreenBtn.style.display = "none";
  ...
  if (user.role === "store") {
    ...
    document.getElementById("storesScreen").classList.remove("hidden");
...
employeeSelect.addEventListener("change", async () => {
  ...
  selectedEmployeeInfo.innerHTML = `<strong>${currentAssetEmployee.name}</strong> — ${currentAssetEmployee.nationalId}`;
...
assetSearchButton.onclick = async () => {
  ...
  selectedEmployeeInfo.className = "message visible info";
  selectedEmployeeInfo.style.display = "block";
  selectedEmployeeInfo.innerHTML = `<strong>${emp.name}</strong> — ${emp.nationalId}`;
...
if (saveCurrentAssetButton) {
  saveCurrentAssetButton.addEventListener("click", () => {
    ...
    if (!openDetails) {
      alert("برجاء فتح نوع العهدة أولاً");
      return;
    }
...
async function editAssetInternal(nid, assetId) {
  ...
  if (!asset) return alert("العهدة غير موجودة");
  ...
  const updated = items.map(a => (a.id === assetId ? { ...a, ...newValues } : a));
...
async function loadEmployeeAssets(nid) {
  ...
  for (let t in grouped) {
    ...
    grouped[t].forEach(a => {
      ...
      if (a.pdfUrl) info += `<a href="${a.pdfUrl}" target="_blank">عرض PDF</a>`;
      info += `</div>`;
...
searchEmployeeButton.onclick = async () => {
  ...
  if (!emp) {
    searchEmployeeInfo.innerHTML = `<span style="color:red">لا يوجد موظف</span>`;
