...
function renderSerialSearchResults(results, totalDocsScanned) {
  ...
  results.forEach(r => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn primary small" onclick="openEmployeeFromSerialSearch('${r.nationalId}')">فتح</button>
      </td>
    `;
...
window.openEmployeeFromSerialSearch = async (nid) => {
  if (!nid) return;
  if (employeeSelect) {
    employeeSelect.value = nid;
    employeeSelect.dispatchEvent(new Event("change"));
    // Hide results after opening employee to reduce clutter
